...
public class EmailService {
    ...
    public void sendValidationMail(String toEmail) {
        ...
        MimeMessage emailForm = createEmailForm(toEmail, ValidationCode);
        redisUtil.setData(toEmail, Integer.toString(ValidationCode), Duration.ofSeconds(300));

        javaMailSender.send(emailForm);
    }

    private MimeMessage createEmailForm(String toEmail, int ValidationCode) {
        MimeMessage mimeMessage = javaMailSender.createMimeMessage();
        MimeMessageHelper mimeMessageHelper = null;
        try {
            mimeMessageHelper = new MimeMessageHelper(mimeMessage, false, "UTF-8");
            mimeMessageHelper.setTo(toEmail);
            mimeMessageHelper.setSubject("[LessonReserve] 회원가입 인증메일");
            String body = "";
            body += "<h1>" + "LessonReserve 회원가입 메일 인증번호입니다." + "</h1>";
